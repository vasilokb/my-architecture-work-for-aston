...
| Backend Monolith | Единое приложение, содержащее всю бизнес-логику. | FastAPI, Python 3.11+, SQLAlchemy, Pydantic | LAN | Модульный монолит из 9 компонентов. Реализован на Python. |
...
|-----------|------------|------------------|-------------------|
| User Management | Управление пользователями и их профилями. | Синхронизация с Keycloak, хранение настроек, ведение истории действий. | FastAPI endpoints, SQLAlchemy models, OAuth2 integration |
...
| File Processor | Извлечение текста из загруженных документов. | Загрузка файлов, парсинг форматов PDF/DOCX/TXT, нормализация и кэширование текста. | python-docx, PyPDF2, textract, boto3 для MinIO |
| Processing Orchestrator | Координация всего процесса анализа документа. | Запуск workflow, управление состояниями, централизованная обработка ошибок от всех компонентов. | FastAPI background tasks, state machine pattern, error handling |
| Queue Manager | Управление асинхронным выполнением задач. | Реализация очереди задач на основе PostgreSQL (SKIP LOCKED), политики повторных попыток (retry). | SQLAlchemy с SKIP LOCKED, async/await, retry patterns |
| LLM Integrator | Взаимодействие с внешними AI-сервисами. | Формирование промптов, вызов API LLM, обработка сетевых ошибок, сохранение сырых ответов. | openai, anthropic библиотеки, httpx для async HTTP |
| Response Validator | Контроль качества и структуры ответов от LLM. | Валидация JSON-схемы, отсев заглушек (Quality Gate), нормализация ролей (всего 5 допустимых). | jsonschema, Pydantic validation, custom quality rules |
| Analysis Engine | Расчёт итоговых оценок трудозатрат. | Расчёт по методу PERT, суммирование по ролям, расчёт бюджета, генерация комментариев. | numpy/pandas для расчётов, математические библиотеки |
| Result Manager | Работа с финальными результатами оценки. | Сохранение, версионирование, экспорт в различные форматы, управление историей изменений. | openpyxl для Excel, reportlab для PDF, versioning logic |
...
|---------|------------------------------|----------------------|
| Выбор FastAPI вместо Django | Требуется высокая производительность для обработки AI-запросов, асинхронность и автоматическая документация OpenAPI. | FastAPI с async/await, автоматическая валидация через Pydantic, встроенная документация Swagger. |
| Использование SQLAlchemy Core вместо ORM для сложных запросов | Сложные аналитические запросы к очередям и состоянию процессов требуют оптимизации производительности. | SQLAlchemy Core для запросов к очередям, SQLAlchemy ORM для простых CRUD операций. |
| Реализация очереди задач на PostgreSQL | Требовалось реализовать асинхронную обработку и механизм повторных попыток без внедрения отдельной брокерской системы (Celery, RabbitMQ). | Таблица processing.task_queue с использованием SKIP LOCKED. Отдельный worker процесс на asyncio. |
| Модульная организация кода | Необходимо чёткое разделение ответственности между компонентами для работы команды стажёров. | Организация по feature-модулям (presales, files, processing, llm, analysis, results). Каждый модуль — отдельный Python пакет. |
| Асинхронная обработка файлов | Загрузка и обработка больших файлов не должна блокировать основной event loop. | Использование async версий библиотек (aiofiles, aioboto3 для MinIO), background tasks в FastAPI. |
...
|-----------|-------------|
| **Фронтенд** | React 18, TypeScript, Vite, Ant Design, React Router |
| **Бэкенд (Python)** | FastAPI 0.104+, Python 3.11+, Uvicorn, Gunicorn |
| **Базы данных** | PostgreSQL 15 (основное хранилище), SQLAlchemy 2.0+, Alembic (миграции), psycopg2 |
| **Объектное хранилище** | MinIO (S3-совместимое), boto3/aioboto3 |
| **Очереди и асинхронность** | PostgreSQL SKIP LOCKED, asyncio, background tasks |
| **Валидация и схемы** | Pydantic 2.0+, jsonschema |
| **AI/ML интеграции** | openai, anthropic библиотеки, httpx для async HTTP |
| **Обработка документов** | PyPDF2, python-docx, textract |
| **Экспорт отчётов** | openpyxl (Excel), reportlab (PDF) |
| **Аутентификация** | Keycloak интеграция через OAuth2, python-jose для JWT |
...
| **Безопасность и доступ** | Корпоративный VPN-шлюз (IPSec/OpenVPN) |
| **Управление зависимостями** | Poetry или pipenv |
| **Тестирование** | pytest, pytest-asyncio, factory_boy |
...
│   │   ├── security.py     # Аутентификация, OAuth2
...
- **Богатая экосистема:** Доступ к широкому спектру библиотек для обработки документов, AI-интеграций и анализа данных.
- **Асинхронность:** Современный async/await подход для эффективной обработки IO-операций (файлы, LLM API).
- **Простота обучения:** Python имеет низкий порог входа, что упрощает onboarding стажёров.
- **Автоматическая документация:** FastAPI генерирует интерактивную документацию OpenAPI/Swagger из кода.

Ключевые архитектурные принципы сохранены:
- **Модульный монолит** для снижения операционной сложности
- **Обязательный VPN** для корпоративной безопасности  
- **PostgreSQL-based очередь** вместо внешних брокеров
- **Многоуровневая валидация** ответов LLM
- **Чёткие границы компонентов** для распределения работы

Архитектура допускает постепенное развитие: переход на Celery для очередей при росте нагрузки, добавление кэширования через Redis, выделение компонентов в микросервисы при необходимости.