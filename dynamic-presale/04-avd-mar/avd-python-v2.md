...
Boundary(LAN, "LAN", "Внутренняя сеть") {
  ...
  Container(backend, "Backend Monolith", "FastAPI/Python", "Модульный монолит, 9 компонентов")
...
| Backend Monolith | Единое приложение, содержащее всю бизнес-логику. | FastAPI, Python 3.11+, SQLAlchemy, Pydantic, Uvicorn | LAN | Модульный монолит из 9 компонентов. Реализован на Python. |
...
title Component Diagram - AI Presale Platform (Python Monolith Structure)

' === ЛЕГЕНДА ===
legend top
  <color:#1E90FF>█</color> Frontend Components
  <color:#FFD700>█</color> Business Logic Components (Python/FastAPI)
...
### 3.2. Описание компонентов (Python реализация)

| Компонент | Назначение | Ключевые функции | Python технологии |
|-----------|------------|------------------|-------------------|
| User Management | Управление пользователями и их профилями. | Синхронизация с Keycloak, хранение настроек, ведение истории действий. | FastAPI endpoints, SQLAlchemy models, OAuth2 integration, python-jose для JWT |
| Presale Manager | Управление жизненным циклом пресейлов. | Создание, чтение, обновление и удаление пресейлов (CRUD), управление статусами и метаданными. | FastAPI CRUD routers, Pydantic schemas, SQLAlchemy ORM |
| File Processor | Извлечение текста из загруженных документов. | Загрузка файлов, парсинг форматов PDF/DOCX/TXT, нормализация и кэширование текста. | python-docx, PyPDF2/pdfplumber, textract, boto3/aioboto3 для MinIO |
| Processing Orchestrator | Координация всего процесса анализа документа. | Запуск workflow, управление состояниями, централизованная обработка ошибок от всех компонентов. | FastAPI background tasks, state machine pattern, error handling, asyncio |
| Queue Manager | Управление асинхронным выполнением задач. | Реализация очереди задач на основе PostgreSQL (SKIP LOCKED), политики повторных попыток (retry). | SQLAlchemy с SKIP LOCKED, async/await, retry patterns, asyncio queues |
| LLM Integrator | Взаимодействие с внешними AI-сервисами. | Формирование промптов, вызов API LLM, обработка сетевых ошибок, сохранение сырых ответов. | openai, anthropic библиотеки, httpx для async HTTP, prompt engineering |
| Response Validator | Контроль качества и структуры ответов от LLM. | Валидация JSON-схемы, отсев заглушек (Quality Gate), нормализация ролей (всего 5 допустимых). | jsonschema, Pydantic validation, custom quality rules, regex patterns |
| Analysis Engine | Расчёт итоговых оценок трудозатрат. | Расчёт по методу PERT, суммирование по ролям, расчёт бюджета, генерация комментариев. | numpy/pandas для расчётов, математические библиотеки, статистические методы |
| Result Manager | Работа с финальными результатами оценки. | Сохранение, версионирование, экспорт в различные форматы, управление историей изменений. | openpyxl для Excel, reportlab для PDF, versioning logic, template rendering |
...
### 5.2. Критические архитектурные решения (Python-специфичные)

| Решение | Проблема, которую оно решает | Реализация на Python |
|---------|------------------------------|----------------------|
| Выбор FastAPI вместо Django/Flask | Требуется высокая производительность для обработки AI-запросов, асинхронность, автоматическая документация OpenAPI и современный async/await подход. | FastAPI с async/await, автоматическая валидация через Pydantic, встроенная документация Swagger, dependency injection. |
| Использование SQLAlchemy 2.0+ с async поддержкой | Необходимость эффективной работы с базой данных в асинхронном контексте и поддержка современных паттернов ORM. | SQLAlchemy 2.0+ с asyncpg драйвером, async session management, declarative mapping. |
| Реализация очереди задач на PostgreSQL (SKIP LOCKED) | Требовалось реализовать асинхронную обработку и механизм повторных попыток без внедрения отдельной брокерской системы (Celery, RabbitMQ) для упрощения инфраструктуры. | Таблица processing.task_queue с использованием SELECT ... FOR UPDATE SKIP LOCKED. Отдельный worker процесс на asyncio с конкурентной обработкой. |
| Модульная организация по feature-модулям | Необходимо чёткое разделение ответственности между компонентами для работы команды стажёров и поддержки кодовой базы. | Организация по бизнес-модулям (users, presales, files, processing, llm, validation, analysis, results). Каждый модуль — отдельный Python пакет с чёткими границами. |
| Асинхронная обработка файлов и HTTP запросов | Загрузка и обработка больших файлов, вызовы внешних API не должны блокировать основной event loop. | Использование async версий библиотек (aiofiles, aioboto3 для MinIO, httpx для HTTP), background tasks в FastAPI, asyncio.gather для параллельных операций. |
| Использование Pydantic для валидации и сериализации | Необходимость строгой валидации данных на всех уровнях приложения (API, бизнес-логика, БД). | Pydantic модели для всех DTO, автоматическая валидация в FastAPI endpoints, кастомные валидаторы для бизнес-правил. |

---

## 6. Технологический стек (Python)

| Категория | Технологии |
|-----------|-------------|
| **Фронтенд** | React 18, TypeScript, Vite, Ant Design, React Router, Axios |
| **Бэкенд (Python)** | FastAPI 0.104+, Python 3.11+, Uvicorn, Gunicorn, Starlette |
| **Базы данных** | PostgreSQL 15, SQLAlchemy 2.0+, Alembic (миграции), asyncpg, psycopg2 |
| **Объектное хранилище** | MinIO (S3-совместимое), boto3, aioboto3 |
| **Очереди и асинхронность** | PostgreSQL SKIP LOCKED, asyncio, concurrent.futures, background tasks |
| **Валидация и схемы** | Pydantic 2.0+, jsonschema, email-validator |
| **AI/ML интеграции** | openai, anthropic, httpx для async HTTP, prompt engineering tools |
| **Обработка документов** | PyPDF2/pdfplumber, python-docx, textract, python-magic |
| **Экспорт отчётов** | openpyxl для Excel, reportlab для PDF, jinja2 для шаблонов |
| **Аутентификация** | Keycloak интеграция через OAuth2, python-jose для JWT, OAuth2lib |
| **Конфигурация** | pydantic-settings, python-dotenv, environment variables |
| **Логирование** | structlog, python-json-logger, loguru |
| **Тестирование** | pytest, pytest-asyncio, factory_boy, hypothesis, coverage |
...
| **Безопасность и доступ** | Корпоративный VPN-шлюз (IPSec/OpenVPN), HTTPS/TLS |
| **Управление зависимостями** | Poetry, pip, virtual environments |
| **Мониторинг** | Prometheus клиент, OpenTelemetry, health checks |

---

## 7. Структура проекта (Python модульный монолит)
...
│   │   ├── security.py     # Аутентификация, OAuth2, JWT
│   │   ├── exceptions.py   # Кастомные исключения
│   │   ├── dependencies.py # FastAPI dependencies
│   │   └── logging.py      # Настройка логирования
│   ├── modules/            # Бизнес-модули (компоненты)
...
│   │   ├── presales/       # Presale Manager
│   │   ├── files/          # File Processor
│   │   ├── processing/     # Processing Orchestrator + Queue Manager
│   │   ├── llm/            # LLM Integrator
│   │   ├── validation/     # Response Validator
│   │   ├── analysis/       # Analysis Engine
│   │   └── results/        # Result Manager
│   └── shared/             # Общие модели, DTO, утилиты
│       ├── __init__.py
...
Представленная архитектура на Python обеспечивает те же функциональные возможности, что и Java-версия, но с преимуществами экосистемы Python:

- **Быстрая разработка:** Python и FastAPI позволяют быстро прототипировать и итерировать, что идеально для команды стажёров.
- **Богатая экосистема:** Доступ к широкому спектру библиотек для обработки документов, AI-интеграций, анализа данных и веб-разработки.
- **Современная асинхронность:** Поддержка async/await для эффективной обработки IO-операций (файлы, LLM API, базы данных).
- **Простота обучения:** Python имеет низкий порог входа и понятный синтаксис, что упрощает onboarding стажёров.
- **Автоматическая документация:** FastAPI генерирует интерактивную документацию OpenAPI/Swagger из кода и типов.
- **Строгая типизация:** Использование TypeScript на фронтенде и Pydantic на бэкенде обеспечивает безопасность типов.

Ключевые архитектурные принципы сохранены:
- **Модульный монолит** для снижения операционной сложности при сохранении чётких границ компонентов
- **Обязательный VPN** для корпоративной безопасности  
- **PostgreSQL-based очередь** вместо внешних брокеров сообщений
- **Многоуровневая валидация** ответов LLM (схема, качество, бизнес-правила)
- **Чёткие границы компонентов** для распределения работы между стажёрами

Архитектура допускает постепенное развитие: переход на Celery/RabbitMQ для очередей при росте нагрузки, добавление кэширования через Redis, выделение компонентов в микросервисы при необходимости, интеграция дополнительных AI-провайдеров и форматов документов.